...
const addProductToCart = () => {
  if (product.value) {
    ...
    const productToAdd = {
      ...
      estimated_volume: product.value.estimated_volume,
      volume_per_unit: product.value.volume_per_unit,
      primary_unit: product.value.primary_unit,
      unit_value: product.value.unit_value,
      unit_label: product.value.unit_label,
      ...
      <div class="container mx-auto px-4 py-8 max-w-full">
        ...
        >
          <div class="product-content flex flex-col lg:flex-row">
            <div class="product-images lg:w-1/2 p-6 flex flex-col gap-3">

              <!-- Главная фотография -->
              <div class="main-image border dark:border-gray-700 rounded-lg overflow-hidden">
                <img
                  :src="mainImage"
                  :alt="product.title"
              ...
              <!-- Миниатюры горизонтально под главной -->
              <div v-if="sortedImages.length > 1" class="thumbnails flex flex-row gap-2 overflow-x-auto pb-1">
                <img
                  v-for="image in sortedImages"
                  :key="image.id"
                  :src="getFullImageUrl(image.image)"
                  :alt="`${product.title} - изображение ${image.order}`"
                  class="h-16 w-16 flex-shrink-0 object-cover rounded cursor-pointer border-2 dark:border-gray-700 hover:border-indigo-500 transition-all"
                  :class="mainImage === getFullImageUrl(image.image) ? 'border-indigo-500' : 'border-transparent'"
            ...
            <div class="product-info lg:w-1/2 p-6 lg:self-start">
              ...
              >
                ...
                <button
                  ...
                  :class="[
                    'w-full sm:w-auto sm:flex-grow sm:ml-4 text-white py-3 px-6 rounded-lg font-semibold transition-all duration-300 flex items-center justify-center',
                    justAdded ? 'bg-emerald-500 just-added-anim' : (buttonPressed ? 'bg-indigo-800 transform scale-95' : 'bg-indigo-600 hover:bg-indigo-700')
                  ]"
                >
                  <ShoppingCart v-if="!justAdded" class="w-5 h-5 mr-2" />
                  <span v-if="justAdded" class="font-bold flex items-center">
...
.thumbnails::-webkit-scrollbar-thumb { background: #d1d5db; border-radius: 99px; }
