...
const tier = computed(() => {
  ...
  // emit рекомендацию наверх каждый раз при смене тира
...
const totalVolume = computed(() =>
    cart.value.reduce((sum, item) => {
        const vol = Number(item.volume_per_unit)
                || Number(item.estimated_volume)
                || 0
        return sum + vol * Number(item.quantity || 0)
    }, 0)
    )
</script>
