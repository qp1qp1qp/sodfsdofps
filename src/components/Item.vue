...
const props = defineProps({
  ...
  estimated_volume: { type: Number, default: 0 },
...
const addToCart = (e) => {
  ...
  const itemToAdd = {
    ...
    estimated_volume: props.estimated_volume,
    ...
    primary_unit: props.primary_unit,
    ...
    <div class="product-info">
      ...
      <div class="product-footer">
        ...
        <button
          ...
          :class="{ added: isAdded, 'just-added-anim': justAdded }"
          :disabled="!is_available || props.quantity === 0"
        >
          <span class="full-cart-text">{{ justAdded ? '✓ Добавлено' : (isAdded ? '+' : 'Купить') }}</span>
          <span class="short-cart-text">{{ justAdded ? '✓' : (isAdded ? '+' : 'Купить') }}</span>
          <span class="tiny-cart-text">{{ justAdded ? '✓' : (isAdded ? '+' : '🛒') }}</span>
        </button>
      </div>
    </div>
  </div>
...
.just-added-anim {
  ...
  background-color: #10b981 !important; /* Зеленый цвет успеха */
}
