      ...
      <template v-for="(item, index) in items" :key="index">
        ...
        <div v-else-if="item.type === 'product'" class="gallery-item size-product">
          <Item
            ...
            :volume_per_unit="item.data.volume_per_unit ? Number(item.data.volume_per_unit) : 0"
            :estimated_volume="item.data.estimated_volume ? Number(item.data.estimated_volume) : 0"
